...
void Simulator::calculateBendingEnergyGradient(int i) {
  ...
  Vec12 res = integrate<Vec12>([&p](float s)->Vec12 {
    ...
    for (int kk = 0; kk < 4; kk++) {
      ans(kk * 3) = 2.f * (cross(1) * (PD(2) * bDD[kk] - PDD(2) * bD[kk]) + cross(2) * (PDD(1) * bD[kk] - PD(1) * bDD[kk])) / PDnorm6;
      ans(kk * 3 + 1) = 2.f * (cross(0) * (PDD(2) * bD[kk] - PD(2) * bDD[kk]) + cross(2) * (PD(0) * bDD[kk] - PDD(0) * bD[kk])) / PDnorm6;
      ans(kk * 3 + 2) = 2.f * (cross(0) * (PD(1) * bDD[kk] - PDD(1) * bD[kk]) + cross(1) * (PDD(0) * bD[kk] - PD(0) * bDD[kk])) / PDnorm6;

      ans.block<3, 1>(kk * 3, 0) += (-6.f * bD[kk] * crossNorm2 / PDnorm8) * PD;
    }

    return ans;
    }, 0, 1);

  gradE.block<12, 1>(index, 0) += res;
}
